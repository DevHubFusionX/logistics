...
function MyAnalytics() {
  ...
  const derivedStats = useMemo(() => {
    ...
    const routeMap = bookings.reduce((acc, b) => {
      ...
      if (!acc[route]) acc[route] = { route, shipments: 0, cost: 0 }
      acc[route].shipments++
    ...
    return {
      ...
      topRoutes: Object.values(routeMap).sort((a, b) => b.shipments - a.shipments).slice(0, 5)
    }
  }, [bookings])
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4 text-center">
        <div className="p-4 bg-red-50 rounded-full">
            <div className="w-8 h-8 text-red-500 font-bold">!</div>
        </div>
                    ...
                    <div key={index} className="space-y-1">
                        ...
                        <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                            <div
                                ...
                                style={{ width: `${ (data.shipments / Math.max(...derivedStats.monthlyData.map(d => d.shipments))) * 100 }% ` }}
                            />
                        </div>
                    </div>
                    ))}
