...
export default function LocationStep({ formData, onChange, onNext }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 w-full">
      <div className="bg-white rounded-none sm:rounded-2xl border-y sm:border border-gray-100 p-5 sm:p-6 w-full">
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              disabled
              className="w-full px-4 py-3 rounded-xl text-sm font-medium bg-gray-50 border border-transparent text-gray-500 cursor-default outline-none"
            >
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full border-2 border-sky-700 inline-block" />
              Delivery city
            </label>
            <select
              value={formData.dropoffLocation.city}
              onChange={(e) => onChange('dropoffLocation', 'city', e.target.value)}
              className="w-full px-4 py-3 rounded-xl text-sm font-medium bg-white border border-gray-200 text-gray-900 focus:border-sky-500 focus:ring-2 focus:ring-sky-100 outline-none transition-all"
            >
        ...
        {formData.dropoffLocation.city && (
          <div className="mt-5 pt-5 border-t border-gray-100 flex items-center gap-3">
            <div className="flex items-center gap-2 px-4 py-2.5 bg-sky-50 rounded-xl flex-1">
              <span className="text-sm font-semibold text-sky-700">Lagos</span>
              <ArrowRight className="w-4 h-4 text-sky-400 flex-shrink-0" />
              <span className="text-sm font-semibold text-sky-700">{formData.dropoffLocation.city}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <button
        type="submit"
        className="w-full flex items-center justify-center gap-2 py-4 bg-sky-700 hover:bg-sky-600 text-white text-sm font-semibold rounded-xl sm:rounded-xl transition-colors"
      >
