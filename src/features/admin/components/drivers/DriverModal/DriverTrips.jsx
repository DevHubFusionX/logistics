...
import { useDriverTripsQuery } from '@/features/admin/hooks/queries/useAdminQueries'
      ...
      <div className="space-y-3">
        {trips.length === 0 ? (
          ...
          trips.map((trip) => {
            ...
            const originVal = trip.pickupLocation?.city || trip.origin
            const destVal = trip.dropoffLocation?.city || trip.destination
            const originStr = typeof originVal === 'object' && originVal !== null
              ? `${originVal.lat?.toFixed(4)}, ${originVal.lng?.toFixed(4)}`
              : originVal || 'Origin'
            const destStr = typeof destVal === 'object' && destVal !== null
              ? `${destVal.lat?.toFixed(4)}, ${destVal.lng?.toFixed(4)}`
              : destVal || 'N/A'
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 text-sm text-gray-700">
                      ...
                      <span className="truncate">{originStr}</span>
                      <ArrowRight className="w-3 h-3 text-gray-300 flex-shrink-0" />
                      <span className="font-semibold text-gray-900 truncate">{destStr}</span>
                    </div>
                  </div>
