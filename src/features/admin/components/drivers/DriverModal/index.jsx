...
import { X, User, Check, AlertTriangle, Loader2, Award, Calendar, FileText } from 'lucide-react'
...
export default function DriverModal({ driver, assignedTruck, activeTab, onTabChange, onClose }) {
  ...
  const { verifyDriver, rejectDriver } = useAdminMutations()
  ...
  const isPending = !driver.isVerified && !driver.isRejected
  const isActionLoading = verifyDriver.isPending || rejectDriver.isPending

  const handleVerify = () => {
    verifyDriver.mutate(driver._id || driver.id, {
      onSuccess: () => {
        onClose()
      }
    })
  ...
  const handleReject = () => {
    ...
    rejectDriver.mutate(
      { id: driver._id || driver.id, reason: rejectReason },
      {
        onSuccess: () => {
        ...
        <div className="relative p-6 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white shrink-0 overflow-hidden">
          ...
          <div className="relative flex items-center justify-between">
            <div className="flex items-center gap-4">
              {/* Avatar */}
              <div className="w-14 h-14 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15 flex items-center justify-center overflow-hidden shadow-lg shrink-0">
                {passportUrl ? (
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="text-xl font-bold tracking-tight text-white">{driver.name}</h2>
                  <div className="flex gap-1.5">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${
                      ...
                      {driver.status === 'active' ? 'Active' : driver.status === 'rejected' ? 'Rejected' : 'Inactive'}
                    </span>
                    {/* Verification Status */}
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${
                ...
                <p className="text-slate-300/80 text-xs mt-1 flex items-center gap-1.5">
                  <span className="font-medium text-sky-300">{driver.email}</span>
          ...
          <nav className="flex gap-1 mt-6 p-1 bg-black/20 backdrop-blur-md rounded-xl border border-white/5">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex-1 py-2 px-3 rounded-lg text-xs font-semibold tracking-wide transition-all uppercase ${
                  activeTab === tab
            ...
            {isRejecting ? (
              <div className="space-y-3">
                ...
                <textarea
                  ...
                  placeholder="e.g. Driver's license is expired. Please re-upload a valid license."
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-amber-800 font-medium">This driver requires verification check.</span>
                </div>
