  ...
  useEffect(() => {
    if (driver && isOpen) {
      setFormData({
        ...
        previousCompany: typeof driver.previousCompany === 'string' ? driver.previousCompany : '',
        ninSlip: null,
        driversLicense: null,
        passportPhoto: null,
        truckDrivingCertification: null,
        ...
        bvn: '' // BVN cannot be updated per backend rules
      })
      setCurrentStep(1)
    } else if (isOpen) {
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        ...
        // Skip bvn on edit — backend rejects it
        if (isEditMode && key === 'bvn') return
