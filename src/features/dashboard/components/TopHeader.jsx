    ...
    <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-40 shadow-sm">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {hasPermission(['Super Admin', 'Support', 'Fleet Officer']) && (
          <div className="relative" ref={quickAddRef}>
            <button
              ...
              className="p-2 rounded-lg bg-sky-700 text-white hover:bg-sky-600 active:bg-sky-800 transition-all shadow-sm hover:shadow"
            ...
            {showQuickAdd && (
              <div className="absolute right-0 top-14 w-56 bg-white rounded-xl shadow-xl border border-gray-200 py-2 z-50 animate-fadeIn">
                ...
                {hasPermission(['Super Admin', 'Support']) && (
                  <button
                    ...
                    className="w-full px-4 py-2.5 text-left hover:bg-sky-50 text-sm flex items-center gap-3 transition-colors"
                  >
        ...
        <div className="relative" ref={notificationsRef}>
          ...
          {showNotifications && (
            <div className="absolute right-0 top-14 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50 animate-fadeIn">
              ...
              <div className="px-4 py-2 border-t border-gray-200">
                <button className="text-sm text-sky-700 hover:text-sky-600 font-medium">View all notifications</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="relative" ref={userMenuRef}>
          ...
          >
            <div className="w-8 h-8 bg-sky-700 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm">
          ...
          {showUserMenu && (
            <div className="absolute right-0 top-14 w-64 bg-white rounded-xl shadow-xl border border-gray-200 py-2 z-50 animate-fadeIn">
              <div className="px-4 py-3 border-b border-gray-100">
                <p className="text-sm font-semibold text-gray-900">{user?.firstName} {user?.lastName}</p>
                <p className="text-xs text-gray-500 mt-0.5">{user?.email}</p>
                <span className="inline-block mt-2 px-2 py-0.5 bg-sky-100 text-sky-700 text-xs font-medium rounded">
                  {user?.role || 'Member'}
                </span>
              </div>
              <button
