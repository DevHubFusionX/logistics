  ...
  return (
    <div className="divide-y divide-gray-100">
      ...
      <div className="p-6 lg:p-10">
        <div className="flex flex-col sm:flex-row sm:items-center gap-6 lg:gap-8">
          <div className="relative group shrink-0 self-center sm:self-auto">
            <div className="w-20 h-20 lg:w-24 lg:h-24 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl flex items-center justify-center text-white text-2xl lg:text-3xl font-black shadow-2xl shadow-blue-200 transition-all duration-500 group-hover:scale-105 group-hover:rotate-3">
              {profileData.avatar ? (
                <img src={profileData.avatar} alt="Profile" className="w-full h-full object-cover rounded-3xl" />
              ) : (
                <span>{(profileData.firstName?.charAt(0) || '')}{(profileData.lastName?.charAt(0) || '') || profileData.email?.charAt(0).toUpperCase()}</span>
              )}
            </div>
            <button className="absolute -bottom-2 -right-2 bg-white p-2 rounded-2xl shadow-xl border border-gray-100 hover:scale-110 active:scale-95 transition-all text-blue-600">
              <Camera className="w-3.5 h-3.5" />
            </button>
          </div>

          <div className="flex-1 space-y-1 lg:space-y-2 text-center sm:text-left">
            <div>
              <h3 className="text-xl lg:text-2xl font-black text-gray-900 tracking-tight leading-none">
                {profileData.firstName} {profileData.lastName}
              </h3>
              <p className="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] mt-2">Verified Professional Account</p>
            </div>
            <p className="text-[11px] lg:text-xs font-medium text-gray-500 leading-relaxed max-w-md mx-auto sm:mx-0">
              Your profile is visible to other enterprise users and logistics partners.
            </p>
          </div>
          ...
          {!isEditing && (
            <button
              onClick={handleEdit}
              className="w-full sm:w-auto px-8 py-3 bg-gray-900 text-white rounded-2xl lg:rounded-[1.2rem] hover:bg-blue-600 transition-all font-black text-[10px] uppercase tracking-widest shadow-xl shadow-gray-200 active:scale-95 shrink-0"
            >
              Modify Secrets
            </button>
          )}
      ...
      <div className="p-8 lg:p-10 space-y-10">
        ...
        {isEditing && (
          <div className="flex flex-col sm:flex-row items-center gap-3 lg:gap-4 pt-10 border-t border-gray-100 sticky bottom-0 bg-white/90 backdrop-blur-md pb-4">
            <button
              onClick={() => {
                setIsEditing(false)
                setEditedData(null)
              }}
              className="w-full sm:w-auto px-8 py-3 rounded-xl hover:bg-gray-50 transition-all font-black text-[10px] uppercase tracking-widest text-gray-400 border border-gray-200"
            >
              Discard Changes
            </button>
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-3 px-10 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 shadow-xl shadow-blue-100 transition-all font-black text-[10px] uppercase tracking-widest disabled:opacity-50 active:scale-95"
            >
