...
function getImports(filePath) {
  ...
  // Strip comments to avoid false positives/negatives
  ...
  // RegEx patterns
  const importExportFromRegex = /(?:import|export)\b[^'"]*?\bfrom\s+['"]([^'"]+)['"]/g;
  ...
  let match;
  while ((match = importExportFromRegex.exec(content)) !== null) {
